...
from editar.utils.drop_path import DropPath
...
class Transformer(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.freqs_cis = precompute_freqs_cis_2d_edit(grid_size, self.config.dim // self.config.n_head, self.config.rope_base, self.cls_token_num)
    ...
    def forward(
        ...
        if self.model_mode == 'joint_cls_emb':
            if (input_img_indices is not None) and (edited_img_indices is not None):
                ...
                input_img_embeddings = self.tok_embeddings(input_img_indices)
                input_img_embeddings = torch.where(force_img_drop_ids[:, None, None], self.img_uncond_embedding, input_img_embeddings)
        ...
        if self.training:
            logits = logits[:, self.block_size + self.cls_token_num - 1:].contiguous()
